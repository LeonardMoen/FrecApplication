...
public class Pacman {
	...
	void movePacman(Board board) {
		...
		if (board.boardMap[row][col] == 6) {
			for (int i = 0; i < Frec.ghosts.length; i++) {
				Frec.ghosts[i].setCanBeEaten(true);
				board.boardMap[row][col] = 0;
			}
		}
		...
		// Check ghost hit
		for (int i = 0; i < Frec.ghosts.length; i++) {
			if (Frec.ghosts[i].row == this.row && Frec.ghosts[i].col == this.col) {
				if (!Frec.ghosts[i].canBeEaten) {
					// If the ghost can eat you
					Frec.sleep(500);
					this.reset();
					this.lives--;
					for (int k = 0; k < Frec.ghosts.length; k++) {
						Frec.ghosts[k].reset();
					}
					if (this.lives <= 0) {
						Frec.lose();
